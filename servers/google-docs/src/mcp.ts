...
const httpPort = portIndex !== -1 ? parseInt(args[portIndex + 1], 10) : 3005;
...
async function main() {
  if (useHttp) {
    ...
    console.error(`[MCP] Google Docs server starting in HTTP mode on port ${httpPort}`);
    await runHttpServer(server, { port: httpPort, name: "google-docs" });
  } else {
    // Stdio mode - traditional subprocess
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error("[MCP] Google Docs server running (stdio)");
  }
}
