...
const httpPort = portIndex !== -1 ? parseInt(args[portIndex + 1], 10) : 3003;
...
async function main() {
  if (useHttp) {
    ...
    console.error(`[MCP] Gmail server starting in HTTP mode on port ${httpPort}`);
    await runHttpServer(server, { port: httpPort, name: "gmail" });
  } else {
    // Stdio mode - traditional subprocess
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error("[MCP] Gmail server running (stdio)");
  }
}
