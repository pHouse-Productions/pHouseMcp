...
const httpPort = portIndex !== -1 ? parseInt(args[portIndex + 1], 10) : 3009;
...
async function main() {
  if (useHttp) {
    ...
    console.error(`[MCP] Telegram server starting in HTTP mode on port ${httpPort}`);
    await runHttpServer(server, { port: httpPort, name: "telegram" });
  } else {
    // Stdio mode - traditional subprocess
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error("[MCP] Telegram server running (stdio)");
  }
}
