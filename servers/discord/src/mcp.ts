...
const httpPort = portIndex !== -1 ? parseInt(args[portIndex + 1], 10) : 3010;
...
async function main() {
  if (useHttp) {
    ...
    console.error(`[MCP] Discord server starting in HTTP mode on port ${httpPort}`);
    await runHttpServer(server, { port: httpPort, name: "discord" });
  } else {
    // Stdio mode - traditional subprocess
    const transport = new StdioServerTransport();
    await server.connect(transport);
    console.error("[MCP] Discord server running (stdio)");
  }
}
