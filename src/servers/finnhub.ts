/**
 * Finnhub MCP Server - Exportable module
...
async function finnhubFetch(apiKey: string, endpoint: string, params: Record<string, string> = {}): Promise<any> {
...
async function getStockQuote(apiKey: string, symbol: string): Promise<StockQuote> {
  const data = await finnhubFetch(apiKey, "/quote", { symbol: symbol.toUpperCase() });
...
async function getCompanyProfile(apiKey: string, symbol: string): Promise<CompanyProfile> {
  const data = await finnhubFetch(apiKey, "/stock/profile2", { symbol: symbol.toUpperCase() });
...
async function getCompanyNews(apiKey: string, symbol: string, daysBack: number = 7): Promise<NewsArticle[]> {
  const today = new Date();
  const from = new Date(today.getTime() - daysBack * 24 * 60 * 60 * 1000);

  const formatDate = (d: Date) => d.toISOString().split('T')[0];

  const data = await finnhubFetch(apiKey, "/company-news", {
    symbol: symbol.toUpperCase(),
...
async function getMarketNews(apiKey: string, category: string = "general"): Promise<NewsArticle[]> {
  const data = await finnhubFetch(apiKey, "/news", { category });
...
async function searchSymbol(apiKey: string, query: string): Promise<Array<{ symbol: string; description: string; type: string }>> {
  const data = await finnhubFetch(apiKey, "/search", { q: query });
...
export interface CreateServerOptions {
  /** Finnhub API key (defaults to FINNHUB_API_KEY env var) */
  apiKey?: string;
}

/**
 * Create and configure the finnhub MCP server.
 */
export async function createServer(options: CreateServerOptions = {}): Promise<Server> {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "get_stock_quote") {
      ...
      try {
        const quote = await getStockQuote(apiKey, symbol);
        return {
          content: [
            {
              type: "text",
    ...
    if (name === "get_company_news") {
      ...
      try {
        const news = await getCompanyNews(apiKey, symbol, Math.min(days_back, 30));
        return {
          content: [
            {
              type: "text",
    ...
    if (name === "search_symbol") {
      ...
      try {
        const results = await searchSymbol(apiKey, query);
        return {
          content: [
            {
              type: "text",
